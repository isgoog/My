...
void WorldSession::HandleAuctionSellItem(WorldPacket & recvData)
{
    ...
    {
        Item* item = items[i];
		uint32 kaiguan = sConfigMgr->GetIntDefault("Auction.kaiguan", 1);
		uint32 jifen = sConfigMgr->GetIntDefault("Auction.jifen", 60000);
        uint32 auctionTime = uint32(etime * sWorld->getRate(RATE_AUCTION_TIME));
        AuctionHouseObject* auctionHouse = sAuctionMgr->GetAuctionsMap(creature->getFaction());
        uint32 deposit = sAuctionMgr->GetAuctionDeposit(auctionHouseEntry, etime, item, finalCount);
        if (!_player->HasEnoughMoney(deposit)|| !_player->GetItemCount(jifen,deposit) && kaiguan == 1)
        {
            SendAuctionCommandResult(0, AUCTION_SELL_ITEM, ERR_AUCTION_NOT_ENOUGHT_MONEY);
            return;
        }
		if (sConfigMgr->GetIntDefault("Auction.kaiguan", 1) == 1)
		_player->DestroyItemCount(jifen , deposit,true);
...
void WorldSession::HandleAuctionPlaceBid(WorldPacket & recvData)
{
	...
	uint32 jifen = sConfigMgr->GetIntDefault("Auction.jifen", 60000);
    if (!player->HasEnoughMoney(price) || !player->GetItemCount(jifen, price) && (sConfigMgr->GetIntDefault("Auction.kaiguan", 1) == 1))
    {
    ...
    if (price < auction->buyout || auction->buyout == 0)
    {
        if (auction->bidder > 0)
        {
			...
			{
				if (sConfigMgr->GetIntDefault("Auction.kaiguan", 1) == 1)
					player->DestroyItemCount(jifen , (price - auction->bid), true);
            ...
            else
            {
				...
				if (sConfigMgr->GetIntDefault("Auction.kaiguan", 1) == 1)
					player->DestroyItemCount(jifen ,price, true);
    ...
    else
    {
		...
		if (player->GetGUIDLow() == auction->bidder)
		{
			...
			if (sConfigMgr->GetIntDefault("Auction.kaiguan", 1) == 1)
				player->DestroyItemCount(jifen , (auction->buyout - auction->bid), true);
		}
        else
        {
            player->ModifyMoney(-int32(auction->buyout));
			if (sConfigMgr->GetIntDefault("Auction.kaiguan", 1) == 1)
				player->DestroyItemCount(jifen,(auction->buyout), true);
    ...
    if (auction && auction->owner == player->GetGUIDLow())
    {
        ...
        if (pItem)
        {
            if (auction->bidder > 0)                        // If we have a bidder, we have to send him the money he paid
            {
                ...
                if (!player->HasEnoughMoney(auctionCut) || !player->GetItemCount(jifen,auctionCut) && (sConfigMgr->GetIntDefault("Auction.kaiguan", 1) == 1))          //player doesn't have enough money, maybe message needed
                    return;
                //some auctionBidderNotification would be needed, but don't know that parts..
                sAuctionMgr->SendAuctionCancelledToBidderMail(auction, trans);
                player->ModifyMoney(-int32(auctionCut));
				if (sConfigMgr->GetIntDefault("Auction.kaiguan", 1) == 1)
					player->DestroyItemCount(jifen , auctionCut, true);
            }
