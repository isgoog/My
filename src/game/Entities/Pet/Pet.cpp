...
{ 
	...
	uint32 OnlyHunter = sConfigMgr->GetIntDefault("BeastMaster.OnlyHunter", 1);
    //Determine pet type
    ...
    if (IsPet() && m_owner->GetTypeId() == TYPEID_PLAYER)
    {
        if ((m_owner->getClass() == CLASS_WARLOCK ||
            ...
            m_owner->getClass() == CLASS_MAGE)&& OnlyHunter==0)             // Water Elemental with glyph
            petType = SUMMON_PET;
        else if (m_owner->getClass() == CLASS_HUNTER || OnlyHunter ==1)
        {
            petType = HUNTER_PET;
            m_unitTypeMask |= UNIT_MASK_HUNTER_PET;
        }
...
{ 
	uint32 VITTFpetKG = sConfigMgr->GetIntDefault("VIP.TFZpetKG", 1);
	uint32 VITTFpet = sConfigMgr->GetIntDefault("VIP.TFZpet", 5);
	...
	if (VITTFpetKG == 1)
	{
		...
		{
			uint32 VIPlevel = player->GetSession()->GetSecurity();
			points += (points + VIPlevel*VITTFpet);
		}
	}
    return points;
}
