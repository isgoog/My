...
void WorldSession::HandleMovementOpcodes(WorldPacket & recvData)
{
	...
	static uint32 checkentry = 0;		//dianti
    ...
    if (movementInfo.flags & MOVEMENTFLAG_ONTRANSPORT)
    {
		...
		if (plrMover->GetMapId() == 548)
		{
			if (Map *tM = mover->GetMap())
			{
				if (GameObject *tTt = tM->GetGameObject(movementInfo.transport.guid))
				{
					if (tTt->IsTransport())
					{
						if (tTt->GetEntry() == 183407 || tTt->GetEntry() == 183490)
						{
							checkentry = tTt->GetEntry();
							//sLog->outString("checkentry is first; %d", checkentry); //测试输出函数
						}
					}
				}
			}
	...
	sLog->outError("cheat", "AC2-%s > time: %d fall-time: %d | xyzo: %f, %f, %fo(%f) flags[%X] opcode[%s] | transport (xyzo): %f, %f, %fo(%f)",
	...
	sLog->outError("cheat", "AC2-%s Transport > GUID: (low)%d - (high)%d",
	...
	if (plrMover)
	{
		if (World::GetEnableMvAnticheat() && !plrMover->IsGameMaster() && !plrMover->GetCharmerOrOwnerPlayerOrPlayerItself()->IsGameMaster() && !plrMover->GetCharmerOrOwnerPlayerOrPlayerItself()->GetVehicle())
		{
			...
			if (!curDest)
			{
				...
				//sLog->outString("allowed_delta : %d , real_delta : %d , tg_z: %d \n", allowed_delta, real_delta, tg_z); //测试输出变量
				...
				if (no_fly_auras && no_swim_in_water && plrMover->m_anti_JumpBaseZ != 0 && JumpHeight < plrMover->m_anti_Last_VSpeed)
				{
					...
					//sLog->outString("反作弊系统提示:: 检测到玩家跳跃不正常 %s (%u)", plrMover->GetName().c_str(), plrMover->GetGUIDLow());
#endif
					check_passed = false;
				...
				*/
				...
				if (real_delta > allowed_delta)
				{
					...
					if (real_delta > 4900.0f)
					{
						...
						sLog->outString("反作弊系统提示:: 检测到玩家传送异常 %s (%u)", plrMover->GetName().c_str(), plrMover->GetGUIDLow());
					}
					else {
						...
						sLog->outString("反作弊系统提示:: 检测到玩家速度异常 %s (%u)", plrMover->GetName().c_str(), plrMover->GetGUIDLow());
					}
#endif
					//sLog->outString("real_delta: %lf , allowed_delta: %lf , tg_z: %lf", real_delta, allowed_delta, tg_z); //测试输出变量
					check_passed = false;
				}
				...
				if (delta_z < plrMover->m_anti_Last_VSpeed && plrMover->m_anti_JumpCount == 0 && tg_z > 2.37f)
				{
					...
					sLog->outString("反作弊系统提示:: 检测到玩家跳跃异常 %s (%u)", plrMover->GetName().c_str(), plrMover->GetGUIDLow());
#endif
					//sLog->outString("delta_z: %lf , tg_z: %lf", delta_z, tg_z); //测试输出变量
					check_passed = false;
				}
				...
				*/
				...
				if (no_waterwalk_auras && !no_waterwalk_flags)
				{
					...
					sLog->outString("反作弊系统提示:: 检测到玩家水上行走异常 %s (%u)", plrMover->GetName().c_str(), plrMover->GetGUIDLow());
					...
					if (const Map *map = plrMover->GetMap())
					{
						...
						if (plane_z > 0.1f || plane_z < -0.1f)
						{
							...
							if (plrMover->m_anti_TeleToPlane_Count > World::GetTeleportToPlaneAlarms())
							{
								...
								sLog->outString("反作弊系统提示:: 检测到玩家平面传送异常 %s (%u)", plrMover->GetName().c_str(), plrMover->GetGUIDLow());
							}
#endif
		...
		if (plrMover->GetMapId() == 548  && !check_passed)
		{
			if (checkentry == 183407 /*|| checkentry == 183490*/)
			{
				check_passed = true;
				checkentry = 0;
			}
		}
		if (plrMover->GetMapId() == 554 && !check_passed)
		{
			check_passed = true;
		}
	}
		...
		if (plrMover && !vehMover)                                           // nothing is charmed, or player charmed
    {
        ...
        plrMover->UpdateFallInformationIfNeed(movementInfo, opcode);
			...
			case 618: underMapValueZ = 28.0f; break; // Ring of Valor
													 //case 548: underMapValueZ = -70.0f; break; // 毒蛇
			default: underMapValueZ = -500.0f; break;
			}

			//sLog->outString("------mops: %lf ppos: %lf", movementInfo.pos.GetPositionZ(), plrMover->GetPositionZ()); //测试输出函数
	...
	else
	{
		...
		//AC2 END
		//sLog->outString("Test on read Here!"); //测试输出
	}
}
