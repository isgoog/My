/*
 * Copyright (C) 2016+     AzerothCore + viste <www.azerothcore.org>, released under GNU GPL v2 license: https://github.com/azerothcore/azerothcore-wotlk/blob/master/LICENSE-GPL2
...
inline uint8 Unit::getRace(bool forceoriginal) const
{
...
void Player::SetFakeRaceAndMorph()
{
       if (getClass() == CLASS_DRUID)
       {
               ...
               else if (getGender() == GENDER_MALE) // HORDE PLAYER, ONLY HAVE MALE Night ELF ID
               {
               ...
               for (Battleground::BattlegroundPlayerMap::const_iterator itr = pBattleGround->GetPlayers().begin(); itr != pBattleGround->GetPlayers().end(); ++itr)
               {
                       if (Player* pPlayer = ObjectAccessor::FindPlayer(itr->first))
                       {
                               if (GetDistance2d(pPlayer->GetPositionX(), pPlayer->GetPositionY()) <= distance)
                               {
                                       ...
                                       if (GetTeamId() == pPlayer->GetTeamId())
                                       {
                                               WorldPacket data;
                                               ChatHandler::BuildChatPacket(data, ChatMsg(msgtype), LANG_UNIVERSAL, this, NULL, message);
                                           pPlayer->GetSession()->SendPacket(&data);
                                       }
                                       else if (msgtype != CHAT_MSG_EMOTE)
                                       {
                                               WorldPacket data;
                                               ChatHandler::BuildChatPacket(data, ChatMsg(msgtype), pPlayer->GetTeamId() == TEAM_ALLIANCE ? LANG_ORCISH : LANG_COMMON, this, NULL, message);
                                               pPlayer->GetSession()->SendPacket(&data);
                                       }
                                       pPlayer->GetSession()->SendPacket(&data);
                               }
...
void Player::FitPlayerInTeam(bool action, Battleground* pBattleGround)
{
       ...
       if (pBattleGround && action)
              SendChatMessage("%sYou are playing for the %s%s in this %s", MSG_COLOR_WHITE, GetTeamId() == TEAM_ALLIANCE ? MSG_COLOR_DARKBLUE"alliance" : MSG_COLOR_RED"horde", MSG_COLOR_WHITE, pBattleGround->GetName());
}
