...
public:
    ...
    void run()
    {
        ...
        sLog->outString("启动反冻结线程 (%u 最大停留时间)...", _delayTime/1000);
        while (!World::IsStopped())
        {
            ...
            {
                sLog->outString("世界线程挂起，启动服务器!");
                ASSERT(false);
            }

            ACORE::Thread::Sleep(1000);
        }
        sLog->outString("反冻结线程无问题:退出.");
    }
};
...
int Master::Run()
{
    ...
    sLog->outString("%s (世界服务器守护进程)", _FULLVERSION);
    ...
    sLog->outString("%s (世界服务器守护进程) ready...", _FULLVERSION);
